...
public class ValidateValueComposite {
    private Map<String, ArrayList<String>> validateMap = new HashMap<String, ArrayList<String>>();
    private ValidateValueManager manager = ValidateValueManager.getInstance();


    public HashMap<String, ErrorValidate> validate(User user) {
        ...
        addValidate(Validator.mobile.getName(),user.getMobile());
        Login login = new Login(user.getUser(),user.getPassword());
    ...
    public HashMap<String, ErrorValidate> validate(UserData user) {
        addValidate(Validator.nif.getName(),user.getNif());
        addValidate(Validator.letterWithSpace.getName(),user.getFirstName());
        addValidate(Validator.letterWithSpace.getName(),user.getLastName());
        addValidate(Validator.adress.getName(), user.getAdress());
    ...
    private HashMap<String, ErrorValidate> generateError() {
        ...
        for (Map.Entry<String, ArrayList<String>> line : this.validateMap.entrySet()) {
            String nameValidador = line.getKey();
            ArrayList<String> lineValue = line.getValue();
            lineValue.forEach((value) -> {
                ValidableValue validable = this.manager.getValidate(nameValidador);
    ...
    private  void addValidate(String key, String value) {
        ...
        if (validateMap.containsKey(key)) {
           ...
           // Por si hay null para esa clave, lo machaco
