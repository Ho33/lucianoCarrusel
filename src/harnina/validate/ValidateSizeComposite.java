...
import harnina.entity.StringLimit;
...
public class ValidateSizeComposite {
    ...
    public HashMap<String, ErrorValidate> validate(Login login) {
        ...
        addValidate(Validator.size.getName(),login.getPassword(),7,12);
        return generateError();
    }
    ...
    private HashMap<String, ErrorValidate> generateError() {
        ...
        for (Map.Entry<String, ArrayList<StringLimit>> line : this.validateMap.entrySet()) {
            ...
            lineValue.forEach((value) -> {
                ValidableLength validable = this.manager.getValidate(nameValidador);
    ...
    private  void addValidate(String key, String control, int min, int max) {
        ...
        if (validateMap.containsKey(key)) {
            ...
            if(temporaryList == null){temporaryList = new ArrayList<StringLimit>();}
